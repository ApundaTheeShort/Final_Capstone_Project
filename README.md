...
Once the containers are running and the `db` service is healthy, apply the database migrations. This must be done _inside_ the `backend` container.
...
- **Django Application:** Open your web browser and go to `http://localhost:8062/`
- **Adminer (Database UI):** Open your web browser and go to `http://localhost:8061/`
  - **System:** MySQL
...
- **Admin Panel:** Access the Django admin panel at `http://localhost:8061/admin/` using your superuser credentials
...
```bash
curl -X POST http://localhost:8062/api/api-token-auth/ \
...
```bash
curl -X GET "http://localhost:8062/api/rooms/?is_available=true&room_type=single" \
...
```bash
curl -X PATCH http://localhost:8062/api/bookings/booking-uuid-here/ \
  -H "Authorization: Token your-auth-token-here" \
  -H "Content-Type: application/json" \
  -d '{"status": "approved"}'
```
...
```bash
curl -X POST http://localhost:8062/api/hostels/ \
  -H "Authorization: Token your-auth-token-here" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X GET "http://localhost:8062/api/bookings/?status=pending" \
  -H "Authorization: Token your-auth-token-here"
```
...
Access the database via Adminer at `http://localhost:8061/` or connect directly:
